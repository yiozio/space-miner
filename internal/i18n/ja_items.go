...
func newJAItems() ItemsStrings {
	return ItemsStrings{
		Parts: map[int]ItemText{
			...
			int(entity.PartIDThrusterStarter): {Name: "スターター スラスター", Desc: "支給品の安価な推進機。最低限の推力。"},
			...
			int(entity.PartIDDroneGun):        {Name: "ガンドローン", Desc: "発射時に自律ドローンを設置する。約10秒間、最寄りの小惑星か敵へ弾を撃つ。"},
		},
